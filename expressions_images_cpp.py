...
public:
    ...
    void init_disp(
        ...
        probe_filter->SetSourceData(mesh);}''')*(u_type=="vtk"))*(im_is_def)+'''

    void eval(
        Eigen::Ref<      Eigen::VectorXd> expr,
        Eigen::Ref<const Eigen::VectorXd> X_  ) const {'''+('''

        std::cout << "X_ = " << X_ << std::endl;''')*(verbose)+(('''

        U->eval(UX, X_);''')*(u_type=="dolfin")+('''
        ...
        interpolator->Interpolate(X_.data(), expr.data());''')*(im_dim==3))*(not im_is_def)+('''
        ...
        std::cout << "expr = " << expr << std::endl;''')*(verbose))*(im_is_def)+'''}
};
...
public:
    ...
    void init_disp(
        ...
        probe_filter->SetSourceData(mesh);}''')*(u_type=="vtk")+'''
    ...
    void update_dynamic_scaling(
        ...
        dynamic_scaling_a = dynamic_scaling[0];
        dynamic_scaling_b = dynamic_scaling[1];}''')*(im_is_def)+'''
