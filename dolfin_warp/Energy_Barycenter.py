...
class BarycentricEnergy(ContinuousEnergy):
    ...
    def __init__(self,
        ...
        self.printer.print_str("Defining elastic barycenter energy…")
        self.printer.inc()
        ...
        self.printer.print_str("Loading shape mappings")

        mappings_fields = [mapping_series.get_mapping(i+1) for i in range(len(mapping_series.filenames))]
        ...
        self.Sigma              = []
        self.P                  = []
        for i in range(len(mapping_series.filenames)): 

            if (self.model == "hooke"):
                kinematics = dmech.LinearizedKinematics(
                    u=mappings_fields[i]-self.problem.U)
                ...
                kinematics = dmech.Kinematics(
                    U=mappings_fields[i]-self.problem.U)
            kinemtics_list.append(kinematics)
            ...
            self.material_list.append(self.material)

            if (self.model == "hooke"):
                self.Psi.append(self.material.psi)
                self.Sigma.append(self.material.sigma)
                self.P.append(self.material.sigma)
            elif (self.model in ("kirchhoff", "neohookean", "mooneyrivlin", "neohookeanmooneyrivlin", "ciarletgeymonat", "ciarletgeymonatneohookean", "ciarletgeymonatneohookeanmooneyrivlin", "ogdenciarletgeymonat", "ogdenciarletgeymonatneohookean", "ogdenciarletgeymonatneohookeanmooneyrivlin")):
                self.Psi.append(self.material.Psi)
                self.Sigma.append(self.material.Sigma)
                self.P.append(self.material.P)

        self.printer.print_str("Defining barycenter energy…")

        self.Psi_V = self.Psi[0]
        for i in range(1,len(self.Psi)):
            self.Psi_V += self.Psi[i]
